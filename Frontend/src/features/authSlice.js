...
export const login = createAsyncThunk("user/login", async(user, thunkAPI) => {
    try {
        ...
        if(error.response){
            const message = error.response.data.message || error.response.data.msg || "Terjadi kesalahan saat login";
            toast.error(message);
...
export const getMe = createAsyncThunk("user/getMe", async(_, thunkAPI) => {
    try {
        ...
        const response = await api.get(process.env.REACT_APP_API_URL + 'api/auth/me', {
        ...
        if (error.response) {
            const message = error.response.data.message || error.response.data.msg || "Terjadi kesalahan";
            // toast.error(message);
            return thunkAPI.rejectWithValue(message);
        } else {
            const message = 'Terjadi kesalahan koneksi';
